...
export default function ResultsPage() {
  ...
  const [games, setGames] = useState<Match[]>(Games); // Estado filtrado
  const [minGoals, setMinGoals] = useState(1); // Valor inicial del slider
  const [isVisible, setIsVisible] = useState(true); // 🔹 Controla la animación de opacida
  const [selectedPlayers, setSelectedPlayers] = useState<string[]>([]);
  const [selectedText, setSelectedText] = useState<string | null>(null);
  const [filtersMobile, setFiltersMobile] = useState<boolean>(false);
  ...
  return (
    <div className="w-full relative flex flex-col min-h-screen bg-bgGames text-white pt-[74px] px-10">
      ...
      <div className="w-full flex pb-5  transition-all duration-300">
        {/* Div de filtros con tamaño fijo */}
        <div
        ...
        >
          <div className={`top-0  ${isActive ? "left-0 " : "left-[-400px]"} absolute transition-all duration-300 pb-4  z-10 w-full h-full `}>
      ...
      <div className={`absolute top-0 left-0 w-full h-screen max-h-screen overflow-scroll overflow-y-scroll bg-bgGames z-20 px-7 transition-all duration-300 ${filtersMobile ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
      <div className="w-full flex justify-end items-center h-[60px]">
          ...
          <div
          className={` h-auto  relative pb-4  overflow-hidden `}
        >
          <div className="w-full flex flex-col items-start space-y-2">
            <h3 className="text-lg font-geistBold  text-white mt-4 mb-4 uppercase "> 
              Filtrar por Jugador
          ...
          <button onClick={()=> setFiltersMobile(false)} className="w-full h-[50px] text-lg font-geistRegular text-white bg-primaryRed rounded-md">APLICAR FILTROS</button>
...
const MatchCard: React.FC<{ match: Match }> = ({ match }) => {
  ...
  return (
    ...
    className="w-full h-[110px] relative overflow-hidden  hover:cursor-pointer border-[1px] border-transparent hover:border-primaryRed rounded-[10px]  transition-all duration-300">
      ...
      <div className="absolute top-2 left-0 w-full flex z-10 justify-center ">{match.isLegit === "L" ? <span className="text-white font-geistRegular text-[13px] bg-indigo-700 border-sm py-[2px] px-1 rounded-lg">Legitimo</span> : <span className="text-white font-geistRegular text-[13px] bg-slate-500 border-sm py-[2px] px-1 rounded-lg" >NO Legitimo</span> }</div>
        <div className="flex gap-[6px] items-center">
          ...
          <Image
            ...
            height={35}
          />{" "}
          <p className="font-geistRegular text-[22px]">{match.LocalResultado}</p>{" "}
          {match.LocalPenalty ? <p className="font-geistRegular text-lg">{`(${match.LocalPenalty})`}</p> : null}
        </div>
        <p>-</p>
        <div className="flex gap-[6px] items-center">
        <p className="font-geistRegular text-[22px]">{match.VisitanteResultado}</p>{" "}
